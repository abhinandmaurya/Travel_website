...
const Navbar = () => {
  ...
  return (
    <>
    <nav id="mainMenu" className="bg-gradient-to-r from-indigo-500 from-10% via-sky-500 via-30% to-emerald-500 to-90% ...">
      <div className="container flex flex-wrap items-center justify-between">
        <Link to="/" className="navbar-brand text-primary font-bold flex items-center">
          <img src="/logo.svg" alt="Sphene Reserves" width={24} height={24}/>
        </Link>
        ...
        >
          <ul className="uppercase text-sm text-gray-700 font-medium flex flex-col mt-4 md:mt-0 md:flex-row md:items-center gap-3">
            ...
            <li className="nav-item">
              <Link
                to="/login"
                className={`nav-link block py-2 px-3 md:inline-flex ${(location.pathname === "/login") ? "text-primary hover:text-primary-700" : "hover:text-primary"}`}
              >
                Login
              </Link>
            </li>
            <li className="nav-item">
              <Link
                to="/register"
                className="nav-link nav-link-btn block bg-primary uppercase text-sm text-white rounded-3xl hover:bg-primary-700 py-2 px-3 text-center transition"
              >
                Reserve a spot
              </Link>
            </li>
